...
検索したい文字列をスペースで繋いで、検索を実行します。この例では、先のタグ付与ルールで利用した文字列を使って検索しています。検索結果は、検索スコアリングの高い順並びます。つまり、上の方がより高い精度での検索結果になります。検索結果をクリックすると、画像がプレビューされます。この時、検索結果の Blob ファイルパスを基に、アップロードした先の Azure Blob Storage から画像ファイルをダウンロードして表示していますので、ネットワーク帯域が小さいと少しもたつく印象を受けると思います。<br/>
