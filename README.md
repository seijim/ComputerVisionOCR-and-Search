...
検索したい文字列をスペースで繋いで、検索を実行します。この例では、先のタグ付与ルールで利用した文字列を使って検索しています。検索結果は、検索スコアリングの高い順に並びます。つまり、上の方がより高い精度での検索結果になります。検索結果をクリックすると、画像がプレビューされます。この時、検索結果の Blob ファイルパスを基に、アップロードした先の Azure Blob Storage から画像ファイルをダウンロードして表示していますので、ネットワーク帯域が小さいと少しもたつく印象を受けると思います。<br/>
<img src="./images/app22.png" /><br/><br/>
検索インデックス スキーマの項目として、タグの配列を格納する項目を用意していますので、自動付与ルールで設定しているタグで検索すると、一番高い検索スコアリングになっています。<br/>
